...
import { getUserData, editUserData,getUserImage } from "../api/auth";
...
const EditInfo = () => {
    ...
    const submitData = (e) => {
        ...
        if (isEmpty(userName) || isEmpty(userAddress) || isEmpty(userPhoneNumber)) {
            setFormData({
            ...
            setFormData({
                ...formData, errorMsg: 'Length must be greater than 10'
            })
        }
        else if (userPhoneNumber.length < 11 || userPhoneNumber.length > 13) {
            setFormData({
                ...formData, errorMsg: 'Phone Number length must be greater than 11 and less than 13'
            })
        }
        ...
        try {
            getUserData(data1.email)
                .then(response => {
                    ...
                    setPicture(response.data.userData.image);                    
    ...
    const showEditUserInfoForm = () => (
        ...
        <form className="signu-form" style={{ "color": "white" }} noValidate>
            ...
            <div className="input-group mb-3">
                ...
                <select className="form-select" required name="country" value={userCountry} onChange={event => setUserCountry(event.target.value)} >
